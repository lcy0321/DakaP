...
async def emoji(ctx: Context):
    ...
    return await count_emojis(guild=ctx.guild, channel=ctx.channel)
...
async def raw(ctx: Context):
    ...
    return await show_raw_message(channel=ctx.channel, message=ctx.message)


@bot.command()
async def choose(ctx: Context, *options: str):
    """Randomly choose the items from the given list."""
    return await random_choice(channel=ctx.channel, options=options)


@bot.command()
async def time(ctx: Context, *time_args: str):
    """Get time in different time zones."""
    return await get_time(channel=ctx.channel, time_args=time_args)
